...
(async () => {
  ...
  if (!CONFIG.figma.apiKey)
    console.log("⚠️  FIGMA_API_KEY 미설정 — 피그마 MCP 비활성화");
  if (!CONFIG.github.token)
    console.log("⚠️  GITHUB_TOKEN 미설정 — PR 자동 생성이 제한될 수 있음");

  await app.start();
  console.log("⚡️ Design Bot v4 (WIZKEY + Figma MCP + Build Verify) 실행 중!");
  console.log(`📁 레포: ${CONFIG.repo.url}`);
  console.log(`🌿 기본 브랜치: ${CONFIG.repo.branch}`);
  console.log(
    CONFIG.figma.apiKey ? "🎨 피그마 MCP: 활성화" : "🎨 피그마 MCP: 비활성화",
  );
  console.log("🔨 빌드 검증: 활성화 (pnpm build)");
  console.log("📝 커밋: /commit 스킬 사용");
  console.log("🔀 PR: /pr 스킬 사용 (Draft)");
  console.log(

...
async function _processCodeRequest({ message, threadId, userName, threadHistory, emit, conversationId, images }) {
  ...
  const stepHandlers = {
      ...
      try {
        ...
        if (docsReady) {
          ...
          if (ctx.docsResult?.implStatus?.status === "implemented") {
            ...
            saveMessage({ conversationId, role: "assistant", content: `이미 구현됨: ${ctx.docsResult.implStatus.implemented?.join(", ")}`, type: "already_implemented", metadata: implResult });
    ...
    generate_code: async () => {
      ...
      const codeResult = await generateCodeChanges(message, ctx.fileContents, ctx.figmaData, ctx.context, repoPath, emit);
    ...
    build: async () => {
      ...
      while (!buildResult.success && buildAttempt < MAX_LOOP) {
        buildAttempt++;
        buildResult = await runBuild(repoPath, emit);
        ...
        try {
          const fixResult = await fixBuildError(buildResult.stderr, currentFiles, repoPath, emit);
...
기술적 용어는 최소화하고, 해결 방향을 간단히 제시해.`,
      ...
      while (!verifyPassed && verifyAttempt < MAX_LOOP) {
        ...
        try {
          const supplementResult = await generateCodeChanges(
            ...
            currentFiles, ctx.figmaData, ctx.context, repoPath, emit,
          );
          ctx.appliedChanges.push(...(supplementResult.appliedChanges || []));

          const rebuildResult = await runBuild(repoPath, emit);
  ...
  saveMessage({ conversationId, role: "assistant", content: summary, type: "success", metadata: { branchName: ctx.branchName, previewUrl: ctx.previewUrl, changedFiles } });
  updateConversation(conversationId, { branch_name: ctx.branchName });
